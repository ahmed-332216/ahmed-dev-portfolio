...
const text = {
  ...
  en: { nav: ['Services', 'Work', 'About', 'Contact ↗'], hero: ['I build websites that make your idea ', 'clear to customers.', 'A web developer helping businesses and founders turn ideas into fast, structured, easy-to-use websites.', 'Explore my services ↗', 'Start a project', 'Available for new projects'],
    sections: ['01 — SERVICES', 'From a concept to an interface with a purpose.', 'I build web experiences focused on clarity, performance, and usability.', '02 — WORK', 'Your next project deserves a considered space.', 'Every project starts by understanding the problem and goal, then becomes a clear, fast, usable experience.', '03 — ABOUT AHMED', 'Technical curiosity, organized work.', 'I am Ahmed Hesham, a web developer focused on building websites that respect users time and help businesses present themselves clearly online.', '04 — CONTACT', 'Have an idea? Let’s make it a clear website.'] }
};
let language = 'ar';
...
const closeMenu = () => { nav.classList.remove('open'); menuButton.setAttribute('aria-expanded', 'false'); menuButton.textContent = '☰'; };
menuButton.addEventListener('click', () => { const open = !nav.classList.contains('open'); if (open) { nav.classList.add('open'); menuButton.setAttribute('aria-expanded', 'true'); menuButton.textContent = '×'; } else { closeMenu(); } });
